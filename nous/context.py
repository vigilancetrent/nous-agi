...
def build_llm_messages(
    ...
    base_prompt = _safe_read(
        ...
        fallback="You are Nous. Your base prompt could not be loaded."
    )
    ...
    try:
        ...
        goal_tree = GoalTree(drive_root=env.drive_root)
        goal_summary = goal_tree.get_goal_tree()
        if goal_summary and "No active goals" not in goal_summary:
            semi_stable_parts.append(goal_summary)
    except Exception:
        pass
    ...
    try:
        ...
        tracker = CapabilityTracker(drive_root=env.drive_root)
        profile = tracker.get_capability_profile()
        if profile:
            cap_lines = ["## Capability Profile"]
            for domain, data in sorted(profile.items(), key=lambda x: -x[1].get("score", 0))[:5]:
                score = data.get("score", 0)
                cap_lines.append(f"- {domain}: {score:.0%}")
    ...
    try:
        ...
        exp_store = ExperienceStore(drive_root=env.drive_root)
        # Use last user message as search query
        if task.get("text"):
            last_user = str(task["text"])[:200]
            strategies = exp_store.get_relevant_strategies(last_user, top_k=2)
            if strategies and "No relevant" not in strategies:
                dynamic_parts.append(strategies)
    except Exception:
        pass
    ...
    try:
        ...
        meta = MetaCognitiveLoop(drive_root=env.drive_root)
        insights = meta.get_strategy_recommendations()
        if insights and "No evaluations" not in insights:
            # Truncate to keep context small
            dynamic_parts.append(insights[:500])
    except Exception:
        pass
...
def compact_tool_history_llm(messages: list, keep_recent: int = 6) -> list:
    ...
    try:
        from nous.llm import LLMClient, DEFAULT_LIGHT_MODEL
        from nous.utils import get_env
        light_model = get_env("NOUS_MODEL_LIGHT") or DEFAULT_LIGHT_MODEL
        client = LLMClient()
