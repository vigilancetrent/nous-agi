...
from nous.llm import LLMClient, normalize_reasoning_effort, add_usage
from nous.tools.registry import ToolRegistry
from nous.context import compact_tool_history, compact_tool_history_llm
from nous.utils import utc_now_iso, append_jsonl, truncate_for_log, sanitize_tool_args_for_log, sanitize_tool_result_for_log, estimate_tokens, get_env
...
def _is_local_llm() -> bool:
    ...
    return bool(base_url) and "openrouter" not in base_url.lower()
...
def _get_pricing() -> Dict[str, Tuple[float, float, float]]:
    ...
    For local LLM, returns all-zero pricing.
    ...
    with _pricing_lock:
        ...
        # Skip OpenRouter pricing fetch when using local LLM
        if not _is_local_llm():
            try:
                from nous.llm import fetch_openrouter_pricing
...
def _estimate_cost(model: str, prompt_tokens: int, completion_tokens: int,
    ...
    """Estimate cost from token counts using known pricing. Returns 0 if model unknown or local."""
    # Local LLM: always free
    if _is_local_llm():
        return 0.0
...
def _drain_incoming_messages(
    ...
    if drive_root is not None and task_id:
        from nous.owner_inject import drain_owner_messages
...
def run_llm_loop(
    ...
    from nous.tools import tool_discovery as _td
    ...
    try:
        ...
        if drive_root is not None and task_id:
            try:
                from nous.owner_inject import cleanup_task_mailbox
