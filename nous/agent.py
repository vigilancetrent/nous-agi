"""
Nous agent core — thin orchestrator.
...
from nous.utils import (
    utc_now_iso, read_text, append_jsonl,
    safe_relpath, truncate_for_log,
    get_git_info, sanitize_task_for_event,
)
from nous.llm import LLMClient, add_usage
from nous.tools import ToolRegistry
from nous.tools.registry import ToolContext
from nous.memory import Memory
from nous.context import build_llm_messages
from nous.loop import run_llm_loop
...
class NousAgent:
    ...
    def handle_task(self, task: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # --- AGI subsystem integration: auto-record experience + metacognition ---
            self._record_task_experience(task, text, usage, llm_trace, start_time)
            ...
            try:
                from nous.tools.browser import cleanup_browser
    ...
    def _record_task_experience(
        ...
        """Auto-record experience and run metacognitive evaluation after each task."""
        try:
            from nous.experience import ExperienceStore, Experience
            store = ExperienceStore(drive_root=self.env.drive_root)

            tools_used = [
                tc.get("name", "") for tc in llm_trace.get("tool_calls", [])
                if isinstance(tc, dict)
            ]
            has_error = text.startswith("⚠️") if text else True
            outcome = "failure" if has_error else "success"

            store.record(Experience(
                task_description=str(task.get("text", ""))[:500],
                task_type=str(task.get("type") or "chat"),
                outcome=outcome,
                tokens_used=int(usage.get("prompt_tokens") or 0) + int(usage.get("completion_tokens") or 0),
                cost=float(usage.get("cost") or 0),
                duration_sec=round(time.time() - start_time, 1),
                tools_used=list(set(tools_used))[:20],
                domain="code" if any(t in tools_used for t in ("repo_write_commit", "run_shell", "git_diff")) else "general",
            ))
        except Exception:
            log.debug("Failed to record experience", exc_info=True)

        try:
            from nous.metacognition import MetaCognitiveLoop
            meta = MetaCognitiveLoop(drive_root=self.env.drive_root)
            meta.evaluate_task(
                task_id=str(task.get("id") or ""),
                task_type=str(task.get("type") or "chat"),
                tokens_used=int(usage.get("prompt_tokens") or 0) + int(usage.get("completion_tokens") or 0),
                rounds_used=int(usage.get("rounds") or 0),
                outcome="failure" if (text or "").startswith("⚠️") else "success",
                tools_used=[tc.get("name", "") for tc in llm_trace.get("tool_calls", []) if isinstance(tc, dict)],
            )
        except Exception:
            log.debug("Failed to run metacognitive evaluation", exc_info=True)
    ...
    def _build_review_context(self) -> str:
        ...
        try:
            from nous.review import collect_sections, compute_complexity_metrics, format_metrics
...
def make_agent(repo_dir: str, drive_root: str, event_queue: Any = None) -> NousAgent:
    env = Env(repo_dir=pathlib.Path(repo_dir), drive_root=pathlib.Path(drive_root))
    return NousAgent(env, event_queue=event_queue)
