"""
Nous — Shared utilities.

Single source for helper functions used across all modules.
Does not import anything from nous.* (zero dependency level).
"""
...
# ---------------------------------------------------------------------------
# Env-var backward compatibility (OUROBOROS_* → NOUS_*)
# ---------------------------------------------------------------------------

def get_env(key: str, default: str = "") -> str:
    """Read NOUS_* env var with OUROBOROS_* fallback + deprecation warning."""
    val = os.environ.get(key)
    if val is not None:
        return val
    if key.startswith("NOUS_"):
        legacy = "OUROBOROS_" + key[len("NOUS_"):]
        val = os.environ.get(legacy)
        if val is not None:
            log.warning("Deprecated env var %s — use %s instead", legacy, key)
            return val
    return default
