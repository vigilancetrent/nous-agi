...
class ToolRegistry:
    ...
    To add a tool: create a module in nous/tools/,
    ...
    def _load_modules(self) -> None:
        """Auto-discover tool modules in nous/tools/ that export get_tools()."""
        import importlib
        import inspect
        import pkgutil
        import nous.tools as tools_pkg
        for _importer, modname, _ispkg in pkgutil.iter_modules(tools_pkg.__path__):
            if modname.startswith("_") or modname == "registry":
                continue
            try:
                mod = importlib.import_module(f"nous.tools.{modname}")
                if hasattr(mod, "get_tools"):
                    fn = mod.get_tools
                    # Support both get_tools() and get_tools(ctx) signatures
                    sig = inspect.signature(fn)
                    if sig.parameters:
                        entries = fn(self._ctx)
                    else:
                        entries = fn()
    ...
    def register(self, entry: ToolEntry) -> None:
        """Register a new tool (for extension by Nous)."""
        self._entries[entry.name] = entry
