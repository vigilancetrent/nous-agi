...
from nous.utils import utc_now_iso
from nous.tools.registry import ToolEntry, ToolContext
...
def _get_llm_url() -> str:
    """Get the LLM chat completions URL from env, defaulting to OpenRouter."""
...
def _is_openrouter() -> bool:
    """Check if we're using OpenRouter based on LLM_BASE_URL."""
    base_url = os.environ.get("LLM_BASE_URL", "https://openrouter.ai/api/v1")
    return "openrouter" in base_url.lower()


OPENROUTER_URL = "https://openrouter.ai/api/v1/chat/completions"  # legacy constant
...
async def _multi_model_review_async(content: str, prompt: str, models: list, ctx: ToolContext):
    ...
    if not api_key:
        return {"error": "LLM_API_KEY / OPENROUTER_API_KEY not set"}
