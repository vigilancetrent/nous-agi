...
from nous.utils import read_text, safe_relpath, utc_now_iso
...
def _summarize_dialogue(ctx: ToolContext, last_n: int = 200) -> str:
    ...
    try:
        ...
        for entry in entries:
            ...
            role = "Creator" if direction == "in" else "Nous"
            text = entry.get("text", "")
        ...
        prompt = f"""Summarize the following dialogue history between the creator and Nous.
...
Now write a comprehensive summary:"""
        ...
        model = get_env("NOUS_MODEL_LIGHT") or DEFAULT_LIGHT_MODEL
...
def get_tools() -> List[ToolEntry]:
    return [
        ...
        ToolEntry("drive_read", {
            "name": "drive_read",
            "description": "Read a UTF-8 text file from Google Drive (relative to MyDrive/Nous/).",
            "parameters": {"type": "object", "properties": {"path": {"type": "string"}}, "required": ["path"]},
        }, _drive_read),
