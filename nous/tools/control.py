...
from nous.utils import utc_now_iso, write_text, run_cmd
...
def _schedule_task(ctx: ToolContext, description: str, context: str = "", parent_task_id: str = "") -> str:
    ...
    if getattr(ctx, 'is_direct_chat', False):
        from nous.utils import append_jsonl
        try:
...
def _switch_model(ctx: ToolContext, model: str = "", effort: str = "") -> str:
    ...
    """
    from nous.llm import LLMClient, normalize_reasoning_effort
...
def get_tools() -> List[ToolEntry]:
    return [
        ...
        ToolEntry("promote_to_stable", {
            ...
            "description": "Promote nous -> nous-stable. Call when you consider the code stable.",
            "parameters": {"type": "object", "properties": {"reason": {"type": "string"}}, "required": ["reason"]},
        }, _promote_to_stable),
        ...
        ToolEntry("toggle_evolution", {
            ...
            "description": "Enable or disable evolution mode. When enabled, Nous runs continuous self-improvement cycles.",
            "parameters": {"type": "object", "properties": {
