...
Communicates with the LLM API (local vLLM or OpenRouter fallback).
...
def fetch_openrouter_pricing() -> Dict[str, Tuple[float, float, float]]:
    ...
    """
    import logging
    log = logging.getLogger("nous.llm")
...
class LLMClient:
    """LLM API wrapper (local vLLM or OpenRouter). All LLM calls go through this class."""
    ...
    def __init__(
        ...
        self._api_key = api_key or os.environ.get(
            "LLM_API_KEY",
            os.environ.get("OPENROUTER_API_KEY", "dummy"),
        )
        self._is_openrouter = "openrouter" in self._base_url.lower()
        self._client = None
    ...
    def _fetch_generation_cost(self, generation_id: str) -> Optional[float]:
        """Fetch cost from OpenRouter Generation API as fallback. Skipped for local LLM."""
        # No generation cost endpoint for local vLLM servers
    ...
    def default_model(self) -> str:
        ...
        return get_env("NOUS_MODEL", "qwen3-coder-next")

    def available_models(self) -> List[str]:
        """Return list of available models from env (for switch_model tool schema)."""
        main = get_env("NOUS_MODEL", "qwen3-coder-next")
        code = get_env("NOUS_MODEL_CODE")
        light = get_env("NOUS_MODEL_LIGHT")
        models = [main]
        if code and code != main:
            models.append(code)
        if light and light != main and light != code:
            models.append(light)
        # Add local model names when not using OpenRouter
        if not self._is_openrouter:
            local_models = [
                "qwen3-coder-next",
                "qwen3-coder",
