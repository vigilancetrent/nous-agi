...
def test_tool_set_matches(registry):
    ...
    assert missing == set(), f"Missing baseline tools: {sorted(missing)}"
...
def test_safe_relpath_normal():
    from nous.utils import safe_relpath
...
def test_clip_text():
    from nous.utils import clip_text
...
def test_estimate_tokens():
    from nous.utils import estimate_tokens
...
def test_memory_identity():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        mem.identity_path().write_text("I am Nous")
        content = mem.load_identity()
        assert "Nous" in content
...
def test_context_build_runtime_section():
    ...
    from nous.context import _build_runtime_section
...
def test_context_build_memory_sections():
    ...
    from nous.context import _build_memory_sections
...
def test_no_hardcoded_replies():
    ...
    violations = []
    for root, dirs, files in os.walk(REPO / "nous"):
...
class TestPrePushGate:
    ...
    def test_run_pre_push_tests_disabled(self):
        """When NOUS_PRE_PUSH_TESTS=0, should return None (skip)."""
        import os
        from nous.tools.git import _run_pre_push_tests
        old = os.environ.get("NOUS_PRE_PUSH_TESTS")
        try:
            os.environ["NOUS_PRE_PUSH_TESTS"] = "0"
            # ctx doesn't matter since we return early
            result = _run_pre_push_tests(None)
            assert result is None
        finally:
            if old is None:
                os.environ.pop("NOUS_PRE_PUSH_TESTS", None)
            else:
                os.environ["NOUS_PRE_PUSH_TESTS"] = old
    ...
    def test_run_pre_push_tests_no_tests_dir(self):
        ...
        old = os.environ.get("NOUS_PRE_PUSH_TESTS")
        try:
            os.environ["NOUS_PRE_PUSH_TESTS"] = "1"
    ...
    def test_git_push_with_tests_exists(self):
        ...
        from nous.tools.git import _git_push_with_tests
