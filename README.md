...
> **New version available:** [Nous Desktop](https://github.com/joi-lab/nous-desktop) — native macOS app with web UI and local model support. This repo is the original Colab/Telegram version.

[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/joi-lab/nous/blob/main/notebooks/quickstart.ipynb)
...
[![GitHub stars](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.github.com%2Frepos%2Fjoi-lab%2Fnous&query=%24.stargazers_count&label=stars&logo=github)](https://github.com/joi-lab/nous/stargazers)
[![GitHub forks](https://img.shields.io/github/forks/joi-lab/nous)](https://github.com/joi-lab/nous/network/members)
...
**Version:** 7.0.0 | [Landing Page](https://joi-lab.github.io/nous/)

---

## What Makes This Different

Most AI agents execute tasks. Nous **creates itself.**
...
CFG = {
    ...
    "GITHUB_REPO": "nous",                                       # <-- repo name (after fork)
    # Models
    "NOUS_MODEL": "anthropic/claude-sonnet-4.6",                 # primary LLM (via OpenRouter)
    "NOUS_MODEL_CODE": "anthropic/claude-sonnet-4.6",            # code editing (Claude Code CLI)
    "NOUS_MODEL_LIGHT": "google/gemini-3-pro-preview",           # consciousness + lightweight tasks
    "NOUS_WEBSEARCH_MODEL": "gpt-5",                             # web search (OpenAI Responses API)
    # Fallback chain (first model != active will be used on empty response)
    "NOUS_MODEL_FALLBACK_LIST": "anthropic/claude-sonnet-4.6,google/gemini-3-pro-preview,openai/gpt-4.1",
    # Infrastructure
    "NOUS_MAX_WORKERS": "5",
...
!git clone https://github.com/joi-lab/nous.git /content/nous_repo
...
**Restarting:** If Colab disconnects or you restart the runtime, just re-run the same cell. Your Nous's evolution is preserved -- all changes are pushed to your fork, and agent state lives on Google Drive.
...
| `GITHUB_REPO` | `nous` | GitHub repository name |
| `NOUS_MODEL` | `anthropic/claude-sonnet-4.6` | Primary LLM model (via OpenRouter) |
| `NOUS_MODEL_CODE` | `anthropic/claude-sonnet-4.6` | Model for code editing tasks |
| `NOUS_MODEL_LIGHT` | `google/gemini-3-pro-preview` | Model for lightweight tasks (dedup, compaction) |
| `NOUS_WEBSEARCH_MODEL` | `gpt-5` | Model for web search (OpenAI Responses API) |
| `NOUS_MAX_WORKERS` | `5` | Maximum number of parallel worker processes |
| `NOUS_BG_BUDGET_PCT` | `10` | Percentage of total budget allocated to background consciousness |
| `NOUS_MAX_ROUNDS` | `200` | Maximum LLM rounds per task |
| `NOUS_MODEL_FALLBACK_LIST` | `google/gemini-2.5-pro-preview,openai/o3,anthropic/claude-sonnet-4.6` | Fallback model chain for empty responses |
...
| `nous` | Your fork | Created at first boot. All agent commits here. |
| `nous-stable` | Your fork | Created at first boot. Crash fallback via `promote_to_stable`. |
...
- **LLM-first task dedup** -- replaced hardcoded keyword-similarity dedup (Bible P3 violation) with light LLM call via NOUS_MODEL_LIGHT. Catches paraphrased duplicates.
...
- **Budget variable unification**: canonical name is `TOTAL_BUDGET` everywhere (removed `NOUS_BUDGET_USD`, fixed hardcoded 1500).
...
- Bible P3: fallback model list configurable via NOUS_MODEL_FALLBACK_LIST env var.
