...
# ---------------------------------------------------------------------------
DRIVE_ROOT: pathlib.Path = pathlib.Path("/content/drive/MyDrive/Nous")
...
# Re-export append_jsonl from nous.utils (single source of truth)
from nous.utils import append_jsonl  # noqa: F401
...
EVOLUTION_BUDGET_RESERVE: float = 0.0  # No reserve — unlimited evolution
...
def check_openrouter_ground_truth() -> Optional[Dict[str, float]]:
    ...
    Skipped when using a local LLM (not OpenRouter).
    """
    # Skip when using local LLM — no OpenRouter API to query
    if _is_local_llm():
        log.debug("Skipping OpenRouter ground truth check — using local LLM")
        return None
...
def update_budget_from_usage(usage: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if cost is None:
            cost = 0.0
        # Local LLM: force cost to zero (free inference)
        if using_local:
            cost = 0.0
    ...
    # Skip drift detection entirely when using local LLM
    if using_local:
        return
