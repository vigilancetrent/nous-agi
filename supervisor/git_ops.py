...
# ---------------------------------------------------------------------------
REPO_DIR: pathlib.Path = pathlib.Path("/content/nous_repo")
...
def ensure_repo_present() -> None:
    ...
    subprocess.run(["git", "config", "user.name", "Nous"], cwd=str(REPO_DIR), check=True)
    subprocess.run(["git", "config", "user.email", "nous@users.noreply.github.com"],
                    cwd=str(REPO_DIR), check=True)
...
def import_test() -> Dict[str, Any]:
    r = subprocess.run(
        ["python3", "-c", "import nous, nous.agent; print('import_ok')"],
        cwd=str(REPO_DIR),
