# ============================
# Nous — Runtime launcher (entry point, executed from repository)
# ============================
...
# ----------------------------
from nous.apply_patch import install as install_apply_patch
from nous.llm import DEFAULT_LIGHT_MODEL
install_apply_patch()
...
# LLM endpoint config: local vLLM by default, OpenRouter as optional fallback
LLM_BASE_URL = get_secret("LLM_BASE_URL", default="https://montana-wagon-codes-quit.trycloudflare.com/v1")
LLM_API_KEY = get_secret("LLM_API_KEY", default="dummy")

# OpenRouter is now optional (fallback only)
...
MAX_WORKERS = int(get_cfg("NOUS_MAX_WORKERS", default="5", allow_legacy_secret=True) or "5")
MODEL_MAIN = get_cfg("NOUS_MODEL", default="anthropic/claude-sonnet-4.6", allow_legacy_secret=True)
MODEL_CODE = get_cfg("NOUS_MODEL_CODE", default="anthropic/claude-sonnet-4.6", allow_legacy_secret=True)
MODEL_LIGHT = get_cfg("NOUS_MODEL_LIGHT", default=DEFAULT_LIGHT_MODEL, allow_legacy_secret=True)

BUDGET_REPORT_EVERY_MESSAGES = 10
SOFT_TIMEOUT_SEC = max(60, int(get_cfg("NOUS_SOFT_TIMEOUT_SEC", default="600", allow_legacy_secret=True) or "600"))
HARD_TIMEOUT_SEC = max(120, int(get_cfg("NOUS_HARD_TIMEOUT_SEC", default="1800", allow_legacy_secret=True) or "1800"))
DIAG_HEARTBEAT_SEC = _parse_int_cfg(
    get_cfg("NOUS_DIAG_HEARTBEAT_SEC", default="30", allow_legacy_secret=True),
    default=30,
    minimum=0,
)
DIAG_SLOW_CYCLE_SEC = _parse_int_cfg(
    get_cfg("NOUS_DIAG_SLOW_CYCLE_SEC", default="20", allow_legacy_secret=True),
    default=20,
    minimum=0,
)
...
os.environ["NOUS_MODEL"] = str(MODEL_MAIN or "anthropic/claude-sonnet-4.6")
os.environ["NOUS_MODEL_CODE"] = str(MODEL_CODE or "anthropic/claude-sonnet-4.6")
if MODEL_LIGHT:
    os.environ["NOUS_MODEL_LIGHT"] = str(MODEL_LIGHT)
...
append_jsonl(DRIVE_ROOT / "logs" / "supervisor.jsonl", {
    ...
    "worker_start_method": str(os.environ.get("NOUS_WORKER_START_METHOD") or ""),
...
while True:
    ...
    for upd in updates:
        ...
        if st.get("owner_id") is None:
            ...
            send_with_budget(chat_id, "✅ Owner registered. Nous online.")
            continue
        ...
        # All other messages (and dual-path commands) → direct chat with Nous
